...
# threshold was an idea to be able to check less rectangles to speed up processing on testing
threshold = 1
...
remove_rects: Set[Tuple[int,int,int,int]] = set()
for (Ax,Ay,Bx,By) in rectangles:
    if not((Ax,By) in inside and (Bx,Ay) in inside):
        remove_rects.add((Ax,Ay,Bx,By))

rectangles.difference_update(remove_rects)
# this got me down to 3000 some rectangles

# I think the next thing to do is to check the edge of each rectangle until one is found with full interior edge


# checking each rectangle largest to smallest
for (Ax,Ay,Bx,By) in sorted(rectangles,key=lambda rec: area(*rec),reverse=True):
    ...
    if not ins:
        continue
    # so far so good, lets now check the entire rectangle edge
    # check set includes every cell on the edge of the rectangle and maybe or maybe not the corners
    check = set()
    for i in range(Ay,By,1 if Ay<=By else -1):
        check.add((Ax,i))
        check.add((Bx,i))
    for i in range(Ax,Bx,1 if Ax<=Bx else -1):
        check.add((i,Ay))
        check.add((i,By))
    ...
    # more option for time saving
    # inside = inside.union(inside_edge)
    # outside = outside.union(check.difference(inside_edge))



# having done all that
# another way to approach the puzzle might be to create a regularly spaced grid of cells
# and then do finer and finer sets on portions of the total area.
# I think every solution like this and the one I did must end with checking the entire edge of rectangles

# I really wish I could visualize the shape, but I am fully aware that a binary array over the whole thing is about 9Gb of space. 9GB if your programming language stores booleans as bytes
